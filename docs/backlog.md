...
| ---- | ---- | ------- | ------ | ----------- | ---- |
| B-01 | Sauvegarde des objets globaux d'un cluster (rôles, tablespaces, extensions) et restauration des propriétaires et des droits — **remonté le 2026-09-22** par `Q-07` (ADR-0016) : c'est ce qui rend la reprise après sinistre réelle, et le couple standard de l'entreprise est `pg_dump` **avec** droits **plus** `pg_dumpall --globals-only`. Lot à fixer par `D-08` | `Q-07`, ADR-0016, recette du lot 2 | Une restauration qui rend l'instance telle qu'elle était, pas seulement ses lignes | Une entrée de configuration de plus, un second artefact par cluster | ouvert |
...
| B-10 | `govulncheck` dans `verify` | Recette du lot 2, 2026-09-22 : au moment d'ajouter une dépendance, l'outil installé était construit avec Go 1.26 et le dépôt exige 1.27, donc **l'analyse n'a pas pu tourner**. La réponse « aucune vulnérabilité » reposait sur les bases d'avis, pas sur notre graphe d'appels réel | Une vulnérabilité d'une dépendance se voit en CI, et seulement si elle est **atteignable** depuis notre code | Faible : un outil de plus dans `mise.toml` et une tâche, à condition qu'il suive la version de Go | ouvert |
