...
| RSV-13 | Un outil d'une **majeure plus récente** que son serveur est compatible et **signalé** : les archives qu'il écrit portent des directives que ce serveur ignore, donc leur restauration là-bas n'est jamais propre. Signalé, pas bloqué — la matrice accepte un client plus récent à dessein, et une distribution ne livre souvent que le plus récent. | `A-15`, `E-007` | `resolve/matrix_test.go › TestRSV13AToolAheadOfItsServerIsSignalled`, `cli/doctor_test.go › TestDoctorSignalsAToolAheadOfItsServer` |
