...
| BKP-06 | Les **sept étapes** de `E-024` s'enchaînent **dans cet ordre**, et le résultat les porte toutes les sept. Une étape qui échoue **arrête** le job et celles d'après ne sont **pas** déclarées faites. **Depuis le lot 3, les sept s'exécutent** : vérification et manifeste ne sont plus déclarées absentes, et la vérification tombe **entre** l'écriture et le manifeste — c'est ce qui permet au manifeste de porter son état. | `E-024`, § 4.1, § 2 `P3` | `backup/service_test.go › TestBKP06TheSevenStepsHappenInTheOrderOfTheSpecification`, `› TestBKP06AFailedStepStopsTheJob`, `› TestBKP06TheSevenStepsAllRunNow`, `› TestTheVerificationHappensBetweenTheWriteAndTheManifest` |
